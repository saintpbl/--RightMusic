...
Page({
  ...
  handleMusicPlay() {
    ...
    // })
    this.musicControl(isplay, musicId, musicmvLink);
    this.videoShow();
  },
  // searchVideo(){
 ...
 //点击暂停/开始
  videoShow:function(){
   ...
   async getSearch(){
    let content=this.data.musicInfo.name+' '+this.data.musicInfo.ar[0].name
    this.setData({
    ...
    let searchResultData = await request('/search', { keywords: content, limit: 20, offset: 0, type:1014 })
    if(searchResultData){
      content=this.data.musicInfo.name
      this.setData({
        searchContent:content
      })
      searchResultData = await request('/search', { keywords: content, limit: 20, offset: 0, type:1014 })
    }
      // console.log(searchResultData);
    searchResultData = searchResultData.result.videos
    searchResult.push(...searchResultData)
    let videoVid=searchResult[0].vid
    let videoType=searchResult[0].type
    this.setData({
       videoVid:videoVid,
       videoType:videoType,
    })
    this.getVideoUrl(videoVid,videoType)
   },
  // toVideo(){
  //    // console.log(event);
  ...
  async musicControl(isPlay, id, link) {
    if (isPlay) {
      if (!link) {
        ...
        // 音乐播放链接
        musicmvLink = musicLinkData.data[0].url;
        if (musicmvLink == null) {
          wx.showToast({
        ...
        backgroundAudioManager.src = musicmvLink;
      }
      backgroundAudioManager.play()
    } else {
      // 暂停
  ...
  async handleSliderChange(e) {
    ...
    if(e.detail.duration){
      this.videoContext.seek(e.detail.value/100*e.detail.duration);
    }
  },
  ...
  async changevideoback(){
    let ischangevideobackground = !this.data.ischangevideobackground
    this.setData({
      ischangevideobackground
    })
    if(ischangevideobackground==true){
      this.ischangevideobackground=false;
    }
    else{
      this.ischangevideobackground=true;
    }

  },
  totest(){
      wx.navigateTo({
      url: '/pages/test/videotest'//?song=' + JSON.stringify(song)
    })
  },
  /**
   * 生命周期函数--监听页面初次渲染完成
